...
print("Number of GPUs:", torch.cuda.current_device())
...
# Create Neptune run
run = neptune.init_run(project='project_name', mode = 'debug', name='domainnet_allconvnet_arch_tune_final',
                         api_token = "api_token")
...
learn_new = Learner(dls_mixed, net, loss_func=CustomLoss(), metrics=accuracy, opt_func=SGD_with_momentum, cbs=[CustomTrackLearningRate()])

#load the pre-trained model
learn_new.load('pass the trained model on domainnet dataset')
...
test_logits_with_ood = concat([out_confidence, test_confidence])
test_labels_with_ood = concat([350 + np.zeros(len(out_confidence)), test_correct])

# for calulating SB-ECE on ood data
ood_logits, ood_labels = learn_new.get_preds()
