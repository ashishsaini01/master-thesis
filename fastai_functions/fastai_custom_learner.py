...
class TrainLearner(Learner):
    ...
    def predict_new_dataset(self, new_dl):
        ...
        with torch.no_grad():
            #for batch in new_dl:
            inputs = new_dl
            inputs = inputs.to(device)  # Move input tensor to the same device as the model parameters
...
class LRFinderCB(Callback):
    def __init__(self, T_max=100, max_mult=3): fc.store_attr()
    
    def before_fit(self, learn):
        self.sched = CosineAnnealingLR(learn.opt, self.T_max)
...
@fc.patch
def lr_find(self:Learner, T_max=100, max_mult=3, start_lr=1e-5, max_epochs=10):
    self.fit(max_epochs, lr=start_lr, cbs=LRFinderCB(T_max=T_max, max_mult=max_mult))
